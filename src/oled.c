...
void display_tm(){
		...
		extern double temp;
	extern double humi;
	extern uint32_t result;//保存红外接收结果
	extern unsigned char swi;
	
	
	sprintf((char *)text,":%.1f",temp/10.0);
		OLED_ShowCHinese(0,0,0);
		OLED_ShowCHinese(16,0,1);
		OLED_ShowCHinese(32,0,2);
		OLED_ShowCHinese(48,0,4);
		OLED_ShowString(64,0,text);
		OLED_ShowCHinese(0,2,0);
		OLED_ShowCHinese(16,2,1);
		OLED_ShowCHinese(32,2,3);
		OLED_ShowCHinese(48,2,4);
	sprintf((char *)text,":%.1f%%",humi);
		OLED_ShowString(64,2,text);
		...
		sprintf((char *)text,"%10s",ts);
		OLED_ShowChar(32,6,':');
		OLED_ShowString(40,6,text);
