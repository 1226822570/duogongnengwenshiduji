...
const DOWNLOAD_CMD_S download_cmd[] =
{
  ...
  {DPID_HONGWAI, DP_TYPE_STRING},
  {DPID_GONGNENG, DP_TYPE_BOOL},
};
...
*/
	extern void w_puter(unsigned char value);
    w_puter(value);                             //串口发送函数
}
...
void all_data_update(void)
{
    ...
    mcu_dp_string_update(DPID_HONGWAI,当前红外读取指针,当前红外读取数据长度); //STRING型数据上报;
    mcu_dp_bool_update(DPID_GONGNENG,当前功能按钮); //BOOL型数据上报;
    */
		mcu_dp_value_update(DPID_TEMP_CURRENT,temp); //VALUE型数据上报;
    mcu_dp_value_update(DPID_HUMIDITY_VALUE,humi); //VALUE型数据上报;
    mcu_dp_string_update(DPID_HONGWAI,ts,sizeof(ts)); //STRING型数据上报;
    mcu_dp_bool_update(DPID_GONGNENG,swi); //BOOL型数据上报;
		display_tm();
}
...
******************************************************************************/
/*****************************************************************************
函数名称 : dp_download_gongneng_handle
功能描述 : 针对DPID_GONGNENG的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_gongneng_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:关/1:开
    unsigned char gongneng;
    extern void led_off();
	extern void led_on();
	extern void send_ir();
    gongneng = mcu_get_dp_download_bool(value,length);
    if(gongneng == 0) {
        //开关关
			swi=0;
			send_ir();
			
				ret = mcu_dp_bool_update(DPID_GONGNENG,0);
    }else {
        //开关开
			swi=1;
			send_ir();
			
				ret = mcu_dp_bool_update(DPID_GONGNENG,1);
    }
...
unsigned char dp_download_handle(unsigned char dpid,const unsigned char value[], unsigned short length)
{
    ...
    ***********************************/
    ...
    switch(dpid) {
        case DPID_GONGNENG:
            //功能按钮处理函数
            ret = dp_download_gongneng_handle(value,length);
        break;
